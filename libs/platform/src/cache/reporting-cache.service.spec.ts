jest.mock('ioredis', () => {
  ...
  const MockRedis = jest.fn(() => mockInstance) as unknown as jest.Mock & {
    mockInstance: typeof mockInstance;
  };
  (MockRedis as unknown as { mockInstance: typeof mockInstance }).mockInstance =
    mockInstance;
...
describe('ReportingCacheService', () => {
  ...
  it('constructor membuat Redis dengan retryStrategy dan lazyConnect false', async () => {
    ...
    const opts = (Redis as unknown as jest.Mock).mock.calls[0]?.[1] as {
    ...
    (
      cache as unknown as { redis: { quit: jest.Mock; disconnect: jest.Mock } }
  ...
  it('read mengembalikan undefined jika dataUpdatedAt bukan string di redis', async () => {
    ...
    (cache as unknown as { redis: unknown }).redis = {
      get: jest
        .fn()
        .mockResolvedValue(
          JSON.stringify({ data: { x: 1 }, dataUpdatedAt: 123 }),
        ),
      status: 'ready',
    };
  ...
  it('read mengembalikan parsed entry jika redis hit valid', async () => {
    ...
    (cache as unknown as { redis: unknown }).redis = {
      get: jest
        .fn()
        .mockResolvedValue(
          JSON.stringify({ data: { v: 1 }, dataUpdatedAt: iso }),
        ),
      status: 'ready',
    };
    await expect(cache.getFresh('good-key')).resolves.toMatchObject({
      data: { v: 1 },
      dataUpdatedAt: iso,
    });
  });
  ...
  it('read fallback memory JSON invalid mengembalikan undefined', async () => {
    ...
    mem.set('reporting:fresh:mem-bad', {
      value: 'not-json',
      expiresAt: Date.now() + 10000,
    });
  ...
  it('read tanpa redis mengembalikan undefined jika JSON invalid di memory', async () => {
    ...
    mem.set('reporting:fresh:no-redis-bad', {
      value: 'not-json',
      expiresAt: Date.now() + 10000,
    });
  ...
  it('write dengan redis set sukses menyimpan lalu getFresh via redis', async () => {
    ...
    (cache as unknown as { redis: unknown }).redis = {
      ...
      set: jest.fn((key: string, raw: string) => {
        store.set(key, raw);
        return 'OK';
      }),
      status: 'ready',
    };
    await cache.getOrLoad('write-redis-success', () =>
      Promise.resolve({ a: 1 }),
    );
    await expect(cache.getFresh('write-redis-success')).resolves.toMatchObject({
      data: { a: 1 },
    });
  });
  ...
  it('loadWithSingleFlight lock acquired lalu cache hit mengembalikan CACHE tanpa loader', async () => {
    ...
    const iso = new Date().toISOString();
    store.set(
      'reporting:fresh:lf-hit',
      JSON.stringify({ data: { v: 99 }, dataUpdatedAt: iso }),
    );
    (cache as unknown as { redis: unknown }).redis = {
    ...
    const result = await cache.getOrLoad('lf-hit', () =>
      Promise.resolve({ v: 0 }),
    );
  ...
  it('loadWithSingleFlight kalah lock polling berhasil setelah holder menulis cache', async () => {
    ...
    const result = await cache.getOrLoad('poll-key', () =>
      Promise.resolve({ v: 999 }),
    );
  ...
  it('loadWithSingleFlight kalah lock dan polling gagal fallback ke loadAndStore', async () => {
    ...
    const result = await cache.getOrLoad('poll-fallback', () =>
      Promise.resolve({ v: 123 }),
    );
    expect(result.source).toBe('COMPUTED');
  ...
  it('acquireLock mengembalikan false saat redis error', async () => {
    ...
    await expect(
      (
        cache as unknown as {
          acquireLock: (k: string, t: string) => Promise<boolean>;
        }
      ).acquireLock('x', 'tok'),
    ).resolves.toBe(false);
  });
  ...
  it('releaseLock tidak throw saat redis eval error', async () => {
    ...
    await expect(
      (
        cache as unknown as {
          releaseLock: (k: string, t: string) => Promise<void>;
        }
      ).releaseLock('x', 'tok'),
