...
describe('InventoryRepository', () => {
  ...
  describe('findByMerchantWithDetails', () => {
    ...
    it('menerapkan filter outletId dan productId jika disediakan', async () => {
      ...
      await repo.findByMerchantWithDetails('mch-001', {
        outletId: 'out-001',
        productId: 'p-001',
      });
  ...
  describe('findByOutletAndProduct', () => {
    it('mengembalikan inventory dari tx client', async () => {
      ...
      const result = await repo.findByOutletAndProduct(
        mockTx as never,
        'out-001',
        'p-001',
      );
  ...
  describe('create', () => {
    it('membuat inventory baru dari tx client', async () => {
      ...
      mockTx.inventory.create.mockResolvedValue(created);

      const result = await repo.create(mockTx as never, data);
  ...
  describe('upsertThreshold', () => {
    it('membuat atau update threshold pada inventory', async () => {
      ...
      const result = {
        ...
        product: { name: 'Kopi', lowStockThreshold: 10 },
        outlet: { name: 'A' },
      };
      mockTx.inventory.upsert.mockResolvedValue(result);
  ...
  describe('clearThreshold', () => {
    it('menghapus lowStockThresholdOverride', async () => {
      const mockTx = makeMockTx();
      mockTx.inventory.updateMany.mockResolvedValue({
        count: 1,
      });

      const result = await repo.clearThreshold(
        mockTx as never,
        'mch-001',
        'out-001',
        'p-001',
  ...
  describe('bulkUpdateQuantityConditional', () => {
    ...
    it('melakukan bulk update untuk beberapa line', async () => {
      ...
      const result = await repo.bulkUpdateQuantityConditional(mockTx as never, {
        ...
        outletId: 'out-001',
        lines: [
          { productId: 'p-001', delta: -2 },
          { productId: 'p-002', delta: -2 },
        ],
      });
