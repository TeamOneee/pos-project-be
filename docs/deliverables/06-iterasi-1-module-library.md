...
> Dokumen ini **melengkapi** `05-iterasi-1-build-plan-nestjs.md` §1 (tech stack) dan §7 (boundary), `06-iterasi-1-module-interface-contract.md` (port/interface antar modul), serta `07-iterasi-1-api-contract.md` (kontrak API) — khusus mendokumentasikan **library/dependency yang dipakai setiap modul** secara eksplisit.
>
...
>
> **Cakupan:** backend saja (9 modul `libs/*` + 2 deployable `apps/api`, `apps/worker`). Frontend (React/Vite) tidak dibahas.
...
- Setiap **bounded context bisnis** menjadi `libs/*` tersendiri: `platform`, `identity`, `tenant`, `catalog`, `inventory`, `sales`, `reporting`, `insight`, `audit` (05 §2).
...
1. **Barrel-only import.** Modul lain hanya boleh `import { CheckoutService } from '@app/sales'` (path alias ke `libs/sales/src/index.ts`), **tidak pernah** ke file internal (05:83).
2. **Interface-first, bukan implementasi.** Pemanggil lintas modul hanya melihat nama interface (mis. `ProductReadPort`, `StockReservationPort`); implementasi berakhiran `Impl` dilarang diimpor (06 §1). Anti-corruption: interface tidak pernah membocorkan entitas Prisma modul lain (06:22).
3. **Anti-corruption boundary.** Modul lain **tidak boleh query tabel modul lain** secara langsung; read lintas modul hanya lewat port yang diekspos pemilik data (05:16).
4. **Shared kernel tunggal.** Satu-satunya shared kernel yang boleh dipakai semua modul adalah **`libs/platform`** — primitif infrastruktur (Prisma, outbox, job, money, pagination, security, error, observability). Platform bukan tempat logic bisnis (06:24).
5. **Komunikasi antar modul** dilakukan dengan 3 mekanisme terstruktur (bukan import bebas):
   ...
   - **domain event** via `@nestjs/event-emitter` (in-process) — khusus untuk Audit;
   - **outbox event** via tabel DB, dikonsumsi worker (async) — untuk reporting (05:17).
...
| `@nestjs/event-emitter` | TBD | Domain event backbone (in-process) | Basis komunikasi async ke Audit (`@OnEvent`) dan publish domain event antar modul (06 §2.2) |
...
> `@prisma/client` tercatat di shared karena **satu instance `PrismaClient` dibangun di `libs/platform`** (`PrismaWriteService` untuk primary, `PrismaReadService` untuk read replica — 05:457–468). Modul bisnis **tidak** membuat client sendiri; mereka mengonsumsi service platform (lihat §3).
...
| `@prisma/client` | TBD | Persistence | `PrismaWriteService` (primary) & `PrismaReadService` (read replica) — 2 instance manual (05:457) | runtime |
...
| `@nestjs/event-emitter` | TBD | Messaging | (opsional) primitif event bersama untuk module publish | runtime |
...
- Healthcheck DB untuk `GET /health` (07 §9.2).
...
| `@nestjs/event-emitter` | TBD | Messaging | Publish `StaffCreatedEvent`, `StaffUpdatedEvent` → Audit (06 §2.2) | runtime |
...
- Tidak ada dependency langsung ke modul bisnis lain; output ke Audit lewat **domain event** (`@nestjs/event-emitter`), bukan pemanggilan langsung (06 §2.2, 05:97).
...
| Library | Versi | Kategori | Fungsi di modul ini | Scope |
|---|---|---|---|---|
| `@nestjs/event-emitter` | TBD | Messaging | Publish `OutletCreatedEvent`, `OutletUpdatedEvent` → Audit | runtime |
...
**Tanggung jawab:** master data kategori, produk, dan harga override per outlet (`ProductReadPort` untuk konsumen harga efektif — FR-CAT-001–012).

| Library | Versi | Kategori | Fungsi di modul ini | Scope |
|---|---|---|---|---|
| `@nestjs/event-emitter` | TBD | Messaging | Publish `PriceChangedEvent`, `ProductStatusChangedEvent` → Audit | runtime |
...
**Tanggung jawab:** manajemen stok per outlet, stock movement, `StockReservationPort` (reservasi stok atomik saat checkout), dan katalog aktif per outlet (FR-INV-001–009).

| Library | Versi | Kategori | Fungsi di modul ini | Scope |
|---|---|---|---|---|
| `@nestjs/event-emitter` | TBD | Messaging | Publish `StockAdjustedEvent` → Audit | runtime |
| `class-validator` / `class-transformer` | TBD | Core | Validasi `AdjustStockDto`, `StockQuery`, dll. | compile, runtime |
| `@prisma/client` | TBD | Persistence | **via platform** — tulis inventory/`stock_movement`; `StockReservationPort` memakai `Prisma.TransactionClient` (atomic conditional update — 05:626) | runtime |
...
| `@nestjs/event-emitter` | TBD | Messaging | Publish `CheckoutCompletedEvent` → Audit | runtime |
| `class-validator` / `class-transformer` | TBD | Core | Validasi `CheckoutDto` (items, payment, discount_pct), `IdempotencyService` status lookup | compile, runtime |
| `@prisma/client` | TBD | Persistence | **via platform** — satu transaksi Prisma: idempotency guard → tulis `transaction`/`transaction_line`/`payment` → publish outbox (05:604–667) | runtime |
| `jest` + `ts-jest` | TBD | Testing | Unit test perhitungan total (discount/service_charge/tax), idempotency, race stok | test |
...
- `identity` — **langsung via interface publik** (data konteks kasir untuk receipt/audit). **Good practice.**
...
**Tanggung jawab:** proyeksi data penjualan (`ReportingProjection`) + query dashboard, dibangun dari event, bukan query tabel mentah (FR-REP-001–010).
...
| `@nestjs/event-emitter` | TBD | Messaging | (opsional) event tambahan bila proyeksi butuh sinkronisasi | runtime |
...
- **TIDAK depend ke `sales`/`inventory`/`catalog`** (anti-corruption — 05:95). Data masuk lewat **outbox event** `TransactionCompletedEvent` yang diproses `ProjectionUpdateService` di worker (idempotent, FR-REP-008). **Good practice.**
...
| `cockatiel` | TBD | Utility | Circuit breaker + retry + timeout untuk provider eksternal agar worker tidak tersumbat (EXT-AI-003, 07:1810) | runtime |
...
- **External AI API** (hanya bila `ExternalAiAdapter` dipakai) — dengan timeout + circuit breaker `cockatiel`. `RuleBasedInsightAdapter` (default) berjalan tanpa resource eksternal (05:798).

---

### 3.8 Audit — `libs/audit`

**Tanggung jawab:** menerima domain event dari semua modul dan menulis `AuditEvent`, plus query riwayat audit (FR-AUD-001–006).

| Library | Versi | Kategori | Fungsi di modul ini | Scope |
|---|---|---|---|---|
| `@nestjs/event-emitter` | TBD | Messaging | `AuditListener` (`@OnEvent`) — subscribe domain event dari modul lain (07:1889) | runtime |
| `class-validator` / `class-transformer` | TBD | Core | Validasi `AuditQuery` | compile, runtime |
| `@prisma/client` | TBD | Persistence | **via platform** — tulis `AuditEvent` lewat `PrismaWriteService` | runtime |
| `jest` + `ts-jest` | TBD | Testing | Unit test listener + mapping action (`STAFF_CREATED`, `CHECKOUT_COMPLETED`, dll.) | test |

**Dependency ke modul lain:**
- `platform` — via interface publik. **Good practice.**
- Modul lain **tidak pernah memanggil audit langsung** (05:97); mereka publish domain event, dan `AuditListener` mendengarkan. **Good practice** (publisher tidak bergantung ke consumer).

**Infrastructure dependency:** PostgreSQL primary (lewat platform). Tidak ada resource eksternal lain.

---

### 3.9 Aplikasi (deployable) — `apps/api` & `apps/worker`
...
|---|---|---|---|
| `identity` | `audit` | Domain event `StaffCreatedEvent` / `StaffUpdatedEvent` via `@nestjs/event-emitter` | Audit tidak boleh dipanggil langsung (05:97); consumer boleh terpisah |
...
| `tenant` | `audit` | Domain event `OutletCreatedEvent` / `OutletUpdatedEvent` | Audit decoupled via event |
| `catalog` | `tenant` | Direct method call via interface publik (`TenantAuthorizationService`) | Isolasi tenant (FR-TEN-010) |
| `catalog` | `audit` | Domain event `PriceChangedEvent` / `ProductStatusChangedEvent` | Audit decoupled via event |
...
| `inventory` | `audit` | Domain event `StockAdjustedEvent` | Audit decoupled via event |
...
| `sales` | `audit` | Domain event `CheckoutCompletedEvent` | Audit decoupled via event |
...
| `reporting` | — | Konsumsi outbox (worker) | Proyeksi dibangun dari event, bukan query tabel `sales` (05:95) |
...
> **Catatan mekanisme async:** jalur async saat ini memakai **outbox table (DB) + `@nestjs/schedule` polling** dan **`@nestjs/event-emitter` (in-process)** — **bukan RabbitMQ** (05 §0: sengaja didefer; Redis/BullMQ juga tidak dipakai di Iterasi 1). Karena semua komunikasi lewat **interface/port**, bila nanti modul di-split menjadi service, jalur ini tinggal mengganti implementasi (mis. ke broker) tanpa mengubah pemanggil.
...
| `audit` | `platform` | interface + domain event |
...
- `reporting`/`insight` **tidak** depend ke `infrastructure` milik `sales`/`inventory`/`catalog` (05:102);
- `audit` tidak dipanggil siapa pun secara langsung (hanya mendengarkan event), sehingga tidak ada cycle melalui audit.
...
3. **Sinkronisasi wajib dalam satu ekosistem:**
   - seluruh `@nestjs/*` (`core`, `common`, `platform-express`, `swagger`, `schedule`, `throttler`, `event-emitter`, `passport`, `jwt`) harus berbagi **versi major yang sama** karena saling tergantung;
